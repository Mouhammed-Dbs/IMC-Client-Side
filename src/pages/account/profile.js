...
export default function Profile({ user }) {
  ...
  const getNumOpenedSessions = (ses) =>
    ses.filter((session) => session.statusFinished === false);
    ...
    <div className="max-h-screen flex flex-col gap-10 overflow-y-scroll pb-20 bg-slate-200">
      ...
      >
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1 mr-2">
              ...
              <ModalBody>
                ...
                <Input
                  ...
                  variant="bordered"
                />
                <Input label="كلمة السر" type="password" variant="bordered" />
              </ModalBody>
              <ModalFooter>
                ...
                <Button
                  color="primary"
                  onPress={onClose}
                  className=" hover:text-blue-500 hover:bg-white hover:border-2 hover:border-blue-500 px-8"
                >
                  تعديل
                </Button>
              </ModalFooter>
